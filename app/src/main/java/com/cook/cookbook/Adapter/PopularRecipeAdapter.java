...
public class PopularRecipeAdapter extends RecyclerView.Adapter<PopularRecipeAdapter.PopularRecipeViewHolder> {

   private final Context context;
   private final ArrayList<Recipe> recipes;

    public PopularRecipeAdapter(Context context, ArrayList<Recipe> recipes) {
    ...
    public PopularRecipeViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new PopularRecipeViewHolder(LayoutInflater.from(context).inflate(R.layout.popular_recipe_item, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull PopularRecipeAdapter.PopularRecipeViewHolder holder, int position) {
            Recipe recipe = recipes.get(position);

            holder.recipeName.setText(recipe.getName());
            holder.recipeDescription.setText(recipe.getDescription().substring(0, 25));
    ...
    public  static class PopularRecipeViewHolder extends RecyclerView.ViewHolder{
        TextView recipeName, recipeDescription;
        ImageView recipeImage;
        public PopularRecipeViewHolder(@NonNull View itemView) {
            super(itemView);
            recipeName = itemView.findViewById(R.id.popular_recipe_name);
            recipeDescription = itemView.findViewById(R.id.popular_recipe_description);
            recipeImage = itemView.findViewById(R.id.popular_recipe_image);
        }
    }
}
